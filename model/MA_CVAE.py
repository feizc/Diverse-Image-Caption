# memory augumented fusion for CLIP-CVAE 
...
class Cond_Attention(GPT2Attention):
    ...
    def forward(self, x, z, layer_past=None, attention_mask=None, head_mask=None):
        ...
        key = torch.cat((key_z, key), dim=-2)
        value = torch.cat((value_z, value), dim=-2)

        attn_outputs = self._attn(query, key, value, attention_mask, head_mask)
        a = attn_outputs[0]

        a = self._merge_heads(a, self.num_heads, self.head_dim)
        a = self.c_proj(a)
...
class Cond_Block(GPT2Block):
    def __init__(self, n_ctx, config, scale=False):
        ...
        nx = config.n_embd
        self.ln_1 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.attn = Cond_Attention(config, scale)
        self.ln_2 = nn.LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = GPT2MLP(4 * nx, config)
...
class Encoder(GPT2Model):
    def __init__(self, config):
        ...
        self.output_attentions = config.output_attentions
        self.output_past = False #config.output_past
        ...
        n = 6  # config.n_layer
        self.h = nn.ModuleList([Unmasked_Block(config) for _ in range(n)])
